...
let cachedData: DolarLinkedHistoricoData | null = null
let cacheTimestamp = 0
const CACHE_DURATION = 3 * 60 * 1000 // 3 minutos

async function fetchDolarLinkedHistoricoData(): Promise<DolarLinkedHistoricoData> {
  try {
    ...
    // Traer el rango C243:G247 de la pestaña "Soberanos"
    const response = await sheets.spreadsheets.values.get({
      spreadsheetId: SPREADSHEET_ID,
      range: "Soberanos!C236:G240",
    })

    const values = response.data.values || []

    // Primera fila son los headers dinámicos
    const headers = values[0] || []
    // El resto son datos
    const dataRows = values.slice(1)

    const items: DolarLinkedHistoricoItem[] = []
    ...
    return {
      ...
      items: items.filter((item) => item.ticker !== ""),
    }
  } catch (error) {
    console.error("Error fetching DOLAR LINKED historico data:", error)
    return {
      headers: {
        col1: "Dur.",
        col2: "Ticker",
...
export async function GET(request: Request) {
  try {
    ...
    const data = await fetchDolarLinkedHistoricoData()
    ...
    console.error("Error in DOLAR LINKED historico API:", error)
    ...
    return NextResponse.json({ error: "Error fetching DOLAR LINKED historico data" }, { status: 500 })
  }
}
